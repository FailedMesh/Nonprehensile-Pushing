...
def get_push_start_POSMAX(push_dir, bottomPos, bottomOrn,
    ...
    max_neg_dist = 100
    for i in range(4):
        dist = targetCornerswrtCOM[:, i].T @ push_vec
        if dist < max_neg_dist:
            max_neg_dist = dist
            best_corner = targetCornerswrtCOM[:, i].T
    # print("Best corner: {}, target_cartesian_pos: {}".format(best_corner, target_cartesian_pos[0:2].T))
    best_corner = target_cartesian_pos[0:2].T + best_corner
    # print("Best corner: {}, target_cartesian_pos: {}".format(best_corner, target_cartesian_pos[0:2].T))

    desired_push_start = np.zeros(shape=(3, ))
    # desired_push_start[0] = best_corner[0] - (GRIPPER_PUSH_RADIUS+0.05)*push_vec[0] # orn_vec[0]
    desired_push_start[0:2] = best_corner - (GRIPPER_PUSH_RADIUS+0.05)*push_vec #[1] 
    desired_push_start [2] = targetPos[2] - 0.01
...
def get_push_start_ORNMAX(push_dir, bottomPos, bottomOrn,
                            targetPos, targetOrn, current_target_obj_size, is_viz=False):
    '''Given the yaw, we get the best push point, that prioritizes the change in position 
    '''
    # bottomYaw = p.getEulerFromQuaternion(bottomOrn)[2] # self.current_bottom_6d_pose[5]
    ...
    min_pos_dist = 100
    for i in range(4):
        dist = np.dot(targetCornerswrtCOM[:, i].T, push_vec)
        if (dist>0) and (dist < min_pos_dist):
            min_pos_dist = dist
            best_corner = targetCornerswrtCOM[:, i].T
    ...
    desired_push_start = np.zeros(shape=(3, ))
    # desired_push_start[0] = best_corner[0] - (GRIPPER_PUSH_RADIUS+0.05)*push_vec[0] # orn_vec[0]
    desired_push_start[0:2] = best_corner[0:2] - (GRIPPER_PUSH_RADIUS+0.05)*push_vec #[1] 
    desired_push_start [2] = targetPos[2] - 0.01
...
def get_push_start2(push_type, push_dir, bottomPos, bottomOrn,
                    targetPos, targetOrn, current_target_obj_size, is_viz=False):
    '''
    Push type: {
        '0': POSMAX,
    ...
    '''
    if push_type==0:
        return get_push_start_POSMAX(push_dir, bottomPos, bottomOrn,
                    targetPos, targetOrn, current_target_obj_size, is_viz=False)
    else:
        return get_push_start_ORNMAX(push_dir, bottomPos, bottomOrn,
                    targetPos, targetOrn, current_target_obj_size, is_viz=False)
