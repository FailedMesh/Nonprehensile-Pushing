...
class Environment:
    ...
    def push2(self, pose0, pose1, speed=0.0002):
        '''No home pose push

        Similar to push function below
        '''
    def push(self, pose0, pose1, speed=0.0002):
        ...
        return success, np.array(all_joints)
    ...
    def grasp(self, pose, angle):
        ...
        if success:
            success = self.straight_move(over, pos, rot, detect_force=True)[0]
        if success:
            self.close_gripper()
            success = self.straight_move(pos, over, rot)[0]
