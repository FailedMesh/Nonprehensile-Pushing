...
class Environment:
    ...
    def straight_move(self, pose0, pose1, rot, speed=0.003, max_force=300, detect_force=False):
        ...
        for n in range(n_push):
            # n = n_push - 1 # Added this change to speed this up, it's stability is yet to be tested (Change: Vishal) - Tracking
            # all_joints = []
            cur_joints = []
       ...
       # print(f"Cur joints shape: {cur_joints.shape}\tAll joints shape: {all_joints.shape}")
        success &= success1
        if len(cur_joints)==0:
            pass
        elif len(all_joints)==0:
    ...
    def push(self, pose0, pose1, speed=0.0002):
       ...
       # print(f"rest_pose: {time.time() - st_time}")

        # print(type(all_joints), all_joints.shape)
        ...
        if success:
           ...
           # print(f"pre-push: {time.time() - st_time}")
            # print(type(cur_joints), cur_joints.shape)
        ...
        if success:
           ...
           # print(f"push: {time.time() - st_time}")
            if len(cur_joints)>0:
                all_joints = np.vstack((all_joints, cur_joints))
            success &= success1
        ...
        if success:
           ...
           # print(f"push end: {time.time() - st_time}")
            if len(cur_joints)>0:
                all_joints = np.vstack((all_joints, cur_joints))
            success &= success1
        ...
        if success:
           ...
           # print(f"post-push: {time.time() - st_time}")
            if len(cur_joints)>0:
                all_joints = np.vstack((all_joints, cur_joints))
            success &= success1
       ...
       # print(f"go home: {time.time() - st_time}")
        if len(cur_joints)>0:
                all_joints = np.vstack((all_joints, cur_joints))
        success &= success1
