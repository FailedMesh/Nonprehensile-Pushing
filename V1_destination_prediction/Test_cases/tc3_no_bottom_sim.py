...
class TestCase1:
    def __init__(self, env):
        ...
        self.bottom_obj_size_ranges = {
            'length': [0.39, 0.4], # [0.35, 0.37], # [0.349, 0.500], # [0.2, 0.21], # # length is along x-axis when orientation yaw = 0
            'width': [0.39, 0.4], # [0.35, 0.37], # [0.250, 0.500],
            'height': [0.08, 0.1]
        }
    ...
    def add_marker_obj(self, obj_pos, obj_orientation, half_extents=np.array([0.08, 0.08, 0.02]) , obj_type='marker'):
        ...
        # marker_obj_id = self.env.client_id.loadURDF("/home/vishal/Volume_E/Active/Undergrad_research/ICRA22/codebases/TEMP/Mid-Level-Planner/Assets/custom/multi_colored.urdf", obj_pos, obj_orientation)
        vuid = self.env.client_id.createVisualShape(p.GEOM_MESH, fileName='./Assets/blocks/multi_colored.obj')
    ...
    def create_specific_test_case(self, obstacle_config, bottom_obj='default', target_obj='test', targetPos=[0, 0]):
        ...
        if target_obj=='train':
            ...
            target_obj_id = self.env.client_id.loadURDF("./Assets/blocks/multi_colored.urdf",target_obj_pos, target_obj_orientation)
    ...
    def sample_test_case(self, bottom_obj='default', target_obj='train'):
        ...
        obstacle1_id = self.create_obj(obstacle1_pos, obstacle1_orientation, half_extents=obstacle1_size/2, obj_type='obstacle')
